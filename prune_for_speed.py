...
def prune_tree_for_speed(tree: Tree, X_val, y_val, task='classification', metric='accuracy', tolerance=0.001):
    ...
    Args:
        ...
        task: 'classification' o 'regression'. En clasificación, la poda mantiene el accuracy (u otra métrica); en regresión, mantiene el error bajo control (por ejemplo, MSE).
        metric: métrica a mantener (por ejemplo, 'accuracy' para clasificación, 'mean_squared_error' para regresión)
        tolerance: tolerancia máxima de pérdida de rendimiento (por ejemplo, 0.001 = 0.1% de pérdida de accuracy, o incremento máximo de error en regresión)
    """
    def post_prune(node):
