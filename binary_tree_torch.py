...
class TNode:
    ...
    def branch(self, modelo, prune, verbose = False, max_branch_depth = float('+inf'), **kwargs):
        ...
        if self._filtro == self._matrix.shape[0]:
            ...
            self._filtro = 0
            self._capa += 1
            if self._capa == len(modelo.fc_layers):
            ...
            new_m = np.vstack((modelo.fc_layers[self._capa].bias.detach().numpy(),
                            modelo.fc_layers[self._capa].weight.detach().numpy())).T
        ...
        if verbose:
            if (TNode.node_id % 100 == 0):
                print(f"Realizado {TNode.node_id/2**(sum([layer.get_weights()[0].shape[1] for layer in modelo.fc_layers]))*100}%", flush = True)
    ...
    def evaluate_input(self, x, max_depth, verbose = False):
        '''
        Calculates the evaluation of the input in the decision tree
        below the node with a depth of max_level.
        ...
        x : numpy.array
            Array of floats.
        max_level : int
            maximum level of recursion in the tree.
        verbose : boolean, optional
            If set to True, information about the final node will be printed.
        ...
        #Find the node which represents the interval (decision rule).
        if evaluation <= 0:
            child_node_id = 0
        else:
            child_node_id = 1
        if len(self._child_nodes) > 0 and max_depth > 0:
            return self._child_nodes[child_node_id].evaluate_input(
                x, max_depth - 1, verbose
            )
        else:
            if verbose:
                print(f"Nodo hoja {self._tag} {self._cat_vector}: {self}  => evaluation = {evaluation}")
            #TODO: CALCULAR CATEGORIA CON +1 salida.
            #TODO: CALCULAR CATEGORÍA CON branching factor > 2
...
class Tree:
    ...
    def create_DT(self, auto_prune = True, verbose: bool = False, **kwargs):
        ...
        m = self.modelo.fc_layers[0].weight.detach().numpy()
        b = self.modelo.fc_layers[0].bias.detach().numpy()
    ...
    def evaluate_input(self, x: np.array, returns = 'all', max_depth = float('+inf'), verbose = False):
        '''
        ...
        '''
        x = np.insert(x, 0, 1)
        output = self.root.evaluate_input(x , max_depth=max_depth, verbose = verbose)
        if returns == 'all':
            return output
        if type(returns) == str:
